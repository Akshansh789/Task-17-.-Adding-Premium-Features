...
const DummyScreen = () => {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      let responce = await fetch(
        'https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key= AIzaSyB0otDOfaFjWxMIU5I63kasL3cmb9j00VU',
        {
          method: 'POST',
          body: JSON.stringify({
  ...
  return (
    <div className='my-2  mx-2 row'>
      ...
      <hr />
      <div>
        <button className="btn btn-primary w-25" onClick={handleVerify}>Verify Your E-mail</button>
      </div>
    </div>
  )
}
